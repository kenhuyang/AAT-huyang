...
unsigned int ang1,ang2; //global var
bool ChangeAng;
...
// All lowpass-filters.
  ...
  float panAngleTemp = (panAngleLP)*(panInverse);
    if(panAngleTemp*panFactor > -panMinPulse && panAngleTemp*panFactor < panMaxPulse) {
      ...
      temp=((float)(temp)-950)/2300*180;
      ServoSupport::SetAng(1,9,temp);
      
      ang1=temp;
      if( ang1<0   ) ang1=0;
      if( ang1>180 ) ang1=179;
      ChangeAng=true;
    }    
  ...
  float tiltAngleTemp = (tiltAngleLP-tiltStart)*(tiltInverse);
    if(tiltAngleTemp*tiltFactor > -tiltMinPulse && tiltAngleTemp*tiltFactor < tiltMaxPulse) {
      ...
      temp=((float)(temp)-950)/2300*180;
      ServoSupport::SetAng(2,10,temp);

      ang2=temp;
      if( ang2<0   ) ang2=0;
      if( ang2>180 ) ang2=179;
      ChangeAng=true;
    }   
  ...
  float rollAngleTemp = (rollAngleLP-rollStart)*(rollInverse);
    if(rollAngleTemp*rollFactor > -rollMinPulse && rollAngleTemp*rollFactor < tiltMaxPulse) {
      ...
      temp=((float)(temp)-950)/2300*180;
      ServoSupport::SetAng(3,8,temp);
    }
   
#if (DEBUG == 0)  
  if( ChangeAng && ! ht_paused ) {
    Serial.println(ang1*180+ang2); //Use BlueTool Send ang1/2
    //Serial.print(ang1);
   // Serial.print("    ");
    //Serial.println(ang2);
  }
